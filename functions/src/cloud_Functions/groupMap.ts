...
export class GroupMap {
  ...
  async priviledgeTN(data: any, context: functions.https.CallableContext) {
    ...
    const groupRef: FirebaseFirestore.DocumentReference = db
      .collection("groups")
      .doc(groupID);
        ...
        .then(dSgroup => {
          ...
          if (groupData !== undefined)
            if ("Priviledge" in groupData) {
              ...
              priviledge[userID] = { DisplayName: displayName, Priviledge: 1 };
            } else {
              priviledge = {
                [userID]: { DisplayName: displayName, Priviledge: 1 }
              };
      ...
      return t
        ...
        .then(dSgroup => {
          ...
          if ("Priviledge" in groupData) {
            ...
            priviledge = {
              userID: { DisplayName: displayName, Priviledge: 3 }
            };
            groupData["Priviledge"] = priviledge;
          }
          return t.update(groupRef, groupData);
      ...
      return t
        ...
        .then(dSgroup => {
          ...
          if (groupData !== undefined)
            if ("Priviledge" in groupData) {
              priviledge = groupData["Priviledge"];
              priviledge[userID] = { DisplayName: displayName, Priviledge: 2 };
            } else {
              priviledge = {
                [userID]: { DisplayName: displayName, Priviledge: 2 }
              };
              groupData["Priviledge"] = priviledge;
            }
          return t.update(groupRef, groupData);
